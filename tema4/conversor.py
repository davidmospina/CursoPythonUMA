...
class Conversor:
    def __init__(self):
        ...
        self.radioKelvin=tk.Radiobutton(self.ventana,text="Kelvin", variable=self.initType, value=1)
        self.radioKelvin.grid(column=0, row=1)
        self.radioCelcius=tk.Radiobutton(self.ventana,text="Celcius", variable=self.initType, value=2)
        self.radioCelcius.grid(column=1, row=1)
        self.radioFahrenheit=tk.Radiobutton(self.ventana,text="Fahrenheit", variable=self.initType, value=3)
    ...
    def calcularConversion(self):
        ...
        if self.finalType[0].get() == True:
            ...
            if self.initType.get() == 2:
                self.valorK = float(self.cantidad) + 273.15
            elif self.initType.get() == 3:
                self.valorK = 5/9*(float(self.cantidad) - 32)+273.15
        ...
        if self.finalType[1].get() == True:
            ...
            if self.initType.get() == 1:
                self.valorC = float(self.cantidad) - 273.15
            elif self.initType.get() == 3:
                self.valorC = (float(self.cantidad) -32)/1.8
        ...
        if self.finalType[2].get() == True:
            ...
            if self.initType.get() == 1:
                self.valorF = float(self.cantidad) * 1.8 + 32
            elif self.initType.get() == 2:
                self.valorF = 1.8*(float(self.cantidad) - 273.15) +32  
